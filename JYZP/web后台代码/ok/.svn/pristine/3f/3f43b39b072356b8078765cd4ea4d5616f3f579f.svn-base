				...
				if(list.size() == 0) {
					...
					if(resultWords != null) {
						...
						try {
							...
							outJsonObject.addProperty("modelId", "312");
							inJsonObject.addProperty("content", resultWords);
							inJsonObject.addProperty("label", "java工程师");
							...
							if(!JsonUtils.parseJsonObject(HttpRequest.postRequest(labelApi, outJsonObject)).toString().contains("errmsg")) {
								 ...
								 * 暂时传的一条数据
								 */
//								JsonArray jsonArray = JsonUtils.parseJsonArray(HttpRequest.postRequest(labelApi, outJsonObject));
//								for(int i = 0; i < jsonArray.size(); i++) {
//									jsonObject = jsonArray.get(i).getAsJsonObject();
//									labelNamesList.add(jsonObject.get("result").getAsString());
//								}
